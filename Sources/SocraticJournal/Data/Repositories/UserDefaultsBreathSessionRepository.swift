...
final class UserDefaultsBreathSessionRepository: BreathSessionRepositoryProtocol, @unchecked Sendable {
    ...
    func getTotalMinutesToday() async throws -> Double {
        let sessions = try await getSessionsForDate(Date())
    ...
    func getStreak() async throws -> Int {
        ...
        let sessionDates = Set(allSessions.map { calendar.startOfDay(for: $0.startedAt) })
        var streak = 0
