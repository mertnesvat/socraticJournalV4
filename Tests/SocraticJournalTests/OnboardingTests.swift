...
struct OnboardingTests {
    ...
    struct ReplayOnboardingTests {
        ...
        @Test("Replay then completes sets flag")
    ...
    struct UserSettingsOnboardingTests {
        ...
        @Test("UserSettings decodes data without onboarding flag")
        func decodesDataWithoutFlag() throws {
            let oldData = """
            {
                "themeMode": "system",
                "dailyGoalMinutes": 5,
                "breathReminderEnabled": false,
                "dailyReminderHour": 7,
            ...
            let settings = try JSONDecoder().decode(UserSettings.self, from: oldData)
            #expect(!settings.hasCompletedOnboarding)
        }
    }
